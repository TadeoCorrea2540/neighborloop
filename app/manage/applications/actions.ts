...
async function loadOwnedApplication(
  ...
  const { data } = await supabase
    .from("applications")
    .select("id, status, mission_id, missions!inner(organization_id, volunteer_capacity)")
    .eq("id", applicationId)
    .maybeSingle();
  const row = data as
    | { status: ApplicationStatus; mission_id: string; missions: { organization_id: string; volunteer_capacity: number | null } | null }
    | null;
  if (!row || !row.missions || row.missions.organization_id !== orgId) return null;
  return { status: row.status, missionId: row.mission_id, capacity: row.missions.volunteer_capacity };
}
